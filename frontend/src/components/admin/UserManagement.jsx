                ...
                <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto pb-2 sm:pb-0">
                    <div className="flex items-center gap-1 bg-white p-1 rounded-lg border border-gray-200 shadow-sm">
                        <button
                            ...
                            className="flex items-center gap-2 px-3 py-2 text-gray-600 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-colors text-sm font-medium"
                        ...
                        <button
                            ...
                            disabled={importing}
                            className="flex items-center gap-2 px-3 py-2 text-gray-600 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-colors text-sm font-medium disabled:opacity-50"
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {filteredUsers.map(user => (
                                <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            {user.custom_data && Object.keys(user.custom_data).length > 0 && (
                                                <div className="mt-1 flex flex-wrap gap-1">
                                                    {Object.entries(user.custom_data).map(([key, value]) => (
                                                        <span key={key} className="inline-flex items-center text-[10px] bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded border border-blue-100">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`capitalize px-2.5 py-0.5 rounded-full border text-xs ${user.role === 'admin' ? 'bg-purple-50 text-purple-700 border-purple-200' :
                                    ...
                                    <td className="px-6 py-4 text-right flex justify-end gap-2">
                                        <button
                                            ...
                                            className="text-gray-500 hover:text-blue-600 hover:bg-gray-50 p-2 rounded transition-colors"
                                            title="Edit User"
                                        >
                                        ...
                                        {user.status === 'active' ? (
                                            ...
                                            <button
                                                onClick={() => toggleUserStatus(user.id, user.status)}
                                                className="text-green-500 hover:text-green-700 hover:bg-green-50 p-2 rounded transition-colors"
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden">
                        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50">
                            <h3 className="font-semibold text-gray-800">{editingUser ? 'Editar Usuário' : 'Criar Usuário'}</h3>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            ...
                            {authFields && authFields.length > 0 && (
                                <div className="border-t border-gray-100 pt-4 mt-2">
                                    <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Campos Personalizados</h4>
                    ...
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden max-h-[90vh] flex flex-col">
                        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="font-semibold text-gray-800">Resumo da Importação</h3>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-green-50 rounded-xl p-4 border border-green-100">
                                    <p className="text-green-600 text-xs font-medium uppercase tracking-wider mb-1">Sucesso</p>
                                    <p className="text-2xl font-bold text-green-700">{importResults.success}</p>
                                </div>
                                <div className={`rounded-xl p-4 border ${importResults.failed > 0 ? 'bg-red-50 border-red-100' : 'bg-gray-50 border-gray-100'}`}>
                                    <p className={`${importResults.failed > 0 ? 'text-red-600' : 'text-gray-500'} text-xs font-medium uppercase tracking-wider mb-1`}>Falha</p>
                                    <p className={`text-2xl font-bold ${importResults.failed > 0 ? 'text-red-700' : 'text-gray-700'}`}>{importResults.failed}</p>
                                </div>
                            </div>
                                            ...
                                            {importResults.errors.map((err, i) => (
                                                <div key={i} className="p-3 text-sm flex gap-3">
                                                    <div className="flex-1">
                                                        <span className="font-semibold text-red-800 block">{err.email || 'Registro Desconhecido'}</span>
                            ...
                            {importResults.createdUsers && importResults.createdUsers.length > 0 && (
                                <div>
                                    ...
                                    <div className="bg-green-50 rounded-xl border border-green-100 overflow-hidden">
                                        <div className="max-h-60 overflow-y-auto divide-y divide-green-100">
                                            {importResults.createdUsers.map((user, i) => (
                                                <div key={i} className="p-3 text-sm flex justify-between items-center text-green-900">
                                                    <span>{user.name}</span>
                                                    <span className="text-green-700 opacity-70 text-xs">{user.email}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
