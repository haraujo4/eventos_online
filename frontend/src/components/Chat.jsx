...
export default function Chat() {
    ...
    return (
        <div className="flex flex-col w-full h-full bg-gray-900 border-l border-gray-800">
            <div className="p-4 border-b border-gray-800 flex items-center justify-between bg-gray-900 z-10">
                <h3 className="font-semibold text-white">Chat ao Vivo</h3>
                ...
                {messages.map((msg) => {
                    ...
                    return (
                        <div key={msg.id} className={`flex flex-col ${msg.userRole === 'system' ? 'items-center my-4' : (isMe ? 'items-end' : 'items-start')}`}>
                            {msg.userRole === 'system' ? (
                                <span className="text-xs text-center text-gray-500 bg-gray-800/50 px-3 py-1 rounded-full">{msg.content}</span>
                            ) : (
                                <div className={`flex gap-3 max-w-[85%] ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 relative ${isMe ? 'bg-blue-500/20' : (isModerator ? 'bg-yellow-500/20' : 'bg-indigo-500/20')
                                        ...
                                        <User className={`w-4 h-4 ${isMe ? 'text-blue-400' : (isModerator ? 'text-yellow-400' : 'text-indigo-400')}`} />
                                        {msg.isHighlighted && (
                                    ...
                                    <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                                        <div className={`flex items-baseline gap-2 ${isMe ? 'justify-end' : 'justify-start'}`}>
                                            <span className={`text-sm font-medium flex items-center gap-1 ${isMe ? 'text-blue-400' : (isModerator ? 'text-yellow-400' : 'text-gray-300')
                                                }`}>
                                                {isMe ? 'Você' : msg.userName}
                                                {isModerator && !isMe && (
                                                    <span className="text-[10px] bg-yellow-400/10 text-yellow-400 px-1.5 rounded uppercase font-bold tracking-wider">
                                                        MOD
                                                    </span>
                                                )}
                                            </span>
                                        ...
                                        <div className={`mt-1 p-3 rounded-2xl text-sm leading-relaxed relative ${msg.isHighlighted ? 'border border-yellow-500/50 shadow-[0_0_15px_rgba(234,179,8,0.1)]' : ''
                                                ...
                                                : (isModerator ? 'bg-gray-800 border border-yellow-500/30 text-gray-200 rounded-tl-none' : 'bg-gray-800 text-gray-300 rounded-tl-none')
                                            }`}>
                                            {msg.isHighlighted && (
            ...
            <div className="p-4 bg-gray-900 border-t border-gray-800 relative">
                {!isChatEnabled && !isModerator ? (
                    <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-lg text-center">
                        <span className="text-red-400 text-sm font-medium">O chat está desativado no momento</span>
                    </div>
                ) : (
                    <>
                        {showEmoji && (
                        ...
                        <form onSubmit={handleSend} className="relative flex items-center gap-2">
                            <button
                                ...
                                className={`p-2 rounded-lg transition-colors ${showEmoji ? 'text-yellow-400 bg-yellow-400/10' : 'text-gray-400 hover:text-gray-300 hover:bg-gray-800'}`}
                            >
                            ...
                            <div className="relative flex-1">
                                <input
                                    ...
                                    className="w-full bg-gray-800 text-white rounded-lg pl-4 pr-12 py-3 focus:outline-none focus:ring-1 focus:ring-blue-500/50 border border-transparent focus:border-blue-500/50 transition-all placeholder-gray-500"
                                />
                                <button
                                    type="submit"
                                    className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 text-blue-500 hover:text-blue-400 hover:bg-blue-500/10 rounded-md transition-colors"
